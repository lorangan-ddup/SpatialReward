...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, required=True, help="Path to data file or directory containing 2pair.json, 3pair.json, 4pair.json")
    ...
    parser.add_argument("--dataset_type", type=str, default="multiedit")
...
def initialize_vllm(args):
    """Initialize vLLM with checkpoint from args"""
...
def main():
    ...
    if args.add_timestamp:
        ...
        if current_output_path.endswith('.json'):
            base_path = current_output_path[:-5]
    ...
    # Main inference loop
    for step, batch_data in enumerate(tqdm(dataloader, disable=bool(int(rank)), desc="Inference")):
    ...
    if int(rank) == 0:
        ...
        print(f"\nCompleted. Results saved to {current_output_path}")
